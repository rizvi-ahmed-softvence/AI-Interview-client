...
const SendNotificationFromAdmin = () => {
      ...
      try {
        ...
        if (response.ok && Array.isArray(userData)) {
          const activeUsers = userData.filter(
            (user) => user.isDeleted === false && user.isBlocked === false
          );
  ...
  const handleSubmit = async (e) => {
    ...
    const body = {
      receiverList: selectedRecipients.includes("all")
        ? "all"
        : selectedRecipients,
  ...
  return (
    <div className="max-w-3xl mx-auto mt-8 mb-10 px-0 md:px-4 lg:px-4">
      ...
      <form
        ...
        className="bg-white text-black  p-6 rounded-lg shadow-md space-y-6"
      >
        {/* Dropdown */}
